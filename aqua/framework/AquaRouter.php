...
use aqua\controllers\ExampleController;
use aqua\Kernel;
...
class AquaRouter
{
    ...
    {
        $this->namespace = 'aqua\\controllers';
    }
    ...
    public function view($pattern, $view, $params = [])
    {
        $this->get($pattern, function () use ($params, $view) {
            echo Kernel::$twig->render($view . '.aqua.php', $params);
        });
    }
    ...
    private function invoke($fn, $params = [])
    {
        ...
        elseif (stripos($fn, '@') !== false) {
            ...
            if (class_exists($controller)) {
                ...
                if (self::response(call_user_func_array([new $controller(), $method], $params)) === false) {
                    // Try to call the method as an non-static method. (the if does nothing, only avoids the notice)
                    if (self::response(forward_static_call_array([$controller, $method], $params)) === false) ;
                }
            }
        }
    }
